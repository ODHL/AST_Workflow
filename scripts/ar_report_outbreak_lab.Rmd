...
rowlabels=merge(rowlabels,sampleDF[,c("specimen_id","run_id","species")])
...
heatmaply(snpData,
          ...
          row_side_colors = rowlabels[,c("run_id","species")],
...
if(exists('treepath')){
  ...
  ggtree_plot <- ggtree_basic + 
    geom_text(data=bs,aes(label=label,hjust=-.4),size=2) + ggplot2::xlim(0, 25)
  ggtree_plot
  ...
  d2 <- sampleDF[,c("specimen_id","bar","colRUN","run_id")]
  d2$specimen_id=as.character(d2$specimen_id)
  sorted_labels=as.data.frame(d)[1:nrow(sampleDF),]
  sorted_labels=sorted_labels[rev(order(sorted_labels$y)),]$label
  d2=d2[match(sorted_labels, d2$specimen_id),]
  d2$specimen_id=factor(d2$specimen_id,levels=rev(d1$specimen_id))
  ...
  p3=ggplot(d2, aes(x =specimen_id, y = bar, fill = specimen_id)) +  
    geom_bar(stat = "identity") +
    ...
    theme(axis.title.x=element_blank(),
        ...
        axis.ticks.y=element_blank())+
    theme(legend.position="none")+
    scale_fill_manual(values=c(rev(d2$colRUN)))
  
  ggarrange(ggtree_plot,p2,p3, widths = c(2,.9,.8),ncol=3)
}
```
