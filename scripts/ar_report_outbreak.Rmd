...
sampleDF=merge(sampleDF,metadataDF[,c("sampleID","Category","Project")], 
...
pass_n=nrow(subset(sampleDF, auto_qc_outcome=="PASS" &Category!="Reference"))
failed_n=nrow(subset(sampleDF, auto_qc_outcome!="PASS" &Category!="Reference" ))
failed_samples=subset(sampleDF, auto_qc_outcome!="PASS"&Category!="Reference")$specimen_id
summaryTEXT=paste0("There were ", nrow(subset(sampleDF,Category!="Reference"))," samples analyzed and ",
...
```{r referenceTEXT, echo=FALSE}
reference_samples=subset(sampleDF,Category=="Reference"&auto_qc_outcome=="PASS" )$specimen_id
ref_n=length(reference_samples)
species_id=unique(subset(sampleDF,Category=="Reference"&auto_qc_outcome=="PASS" )$species)
referenceTEXT=paste0("Several reference samples, not related to the outbreak, were included for comparison. These samples were randomly chosen from previous AR projects, all belonging to the same suspected species identification (",species_id,"). These (", ref_n,") samples included: ",paste(reference_samples,collapse=" | "), ". They are identified in subsequent data tables and figures.")
```
...
collabels=merge(collabels,sampleDF[,c("specimen_id","Category")])
...
heatmaply(snpData,
          ...
          col_side_colors = data.frame("Category" = collabels[,c("Category")],
                                       check.names=FALSE),
          row_side_colors = data.frame("Species" = rowlabels[,c("species")],
                                       check.names=FALSE),
...
x<-fastBM(tree)
obj<-contMap(tree,x)
plot(obj,type="fan",lwd=2)

# tree_pruned <- drop.tip(tree, "23AR000059")
# x_pruned<-fastBM(tree_pruned)
# obj_pruned<-contMap(tree_pruned,x_pruned)
# plot(obj_pruned,type="fan",lwd=2) ## for example
...
rownames(sampleDF)=sampleDF$specimen_id
sampleDF$ref=sampleDF$Category
sampleDF$ref=gsub("Reference","R",sampleDF$ref)
sampleDF$ref=gsub("Outbreak","O",sampleDF$ref)
labels=p$data$label[p$data$label %ni% bs$label]
d <- data.frame(label = labels, 
                  ref  = sampleDF[labels, c("ref")],
                  species = sampleDF[labels, c("species")])
...
DT::datatable(data.frame(ID=c(1:length(ids)),
                         wgs_ID=ids,
                         Category=sampleDF[ids,"Category"]))
