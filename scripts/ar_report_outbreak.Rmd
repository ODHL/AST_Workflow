...
reference_samples=subset(sampleDF,Category=="Reference")$specimen_id
ref_n=length(reference_samples)
species_id=unique(subset(sampleDF,Category=="Reference")$species)
...
PHYLO_DF<-function(select_col,color_col){
  #select_col="Category";color_col="colCat"
  sub_df=subset(sampleDF,auto_qc_outcome=="PASS")
  d1 <- sub_df[,c("specimen_id","bar",color_col,select_col)]
  d1$specimen_id=as.character(d1$specimen_id)
  sorted_labels=as.data.frame(d)[1:nrow(sub_df),]
...
if(exists('treepath')){
  ...
  ggtree_plot <- ggtree_basic + 
    ...
    ggplot2::xlim(0, (tree$Nnode+.5))
  ggtree_plot

  # create df for bar labels
  sampleDF$bar=1
  phylo_df=PHYLO_DF("Category","colCat")
  p1=ggplot(phylo_df, aes(x =specimen_id, y = bar, fill = Category)) +  
  ...
  phylo_df=PHYLO_DF("species","colID")
  p2=ggplot(phylo_df, aes(x =specimen_id, y = bar, fill = specimen_id)) +  
    geom_bar(stat = "identity") +
    ...
    theme(axis.title.x=element_blank(),
        ...
        axis.ticks.y=element_blank())+
    theme(legend.position="none")+
    scale_fill_manual(values=c(rev(phylo_df$colID)))
